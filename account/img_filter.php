<?php
function img_filter($img_name,$img_tm_name,$location,$alert,$message,$multi){
	if ($multi==1){
		$i=0;
		...
		$imageinfo = getimagesize($img_tm_name);
		if( $imageinfo['mime']!='image/gif' && $imageinfo['mime']!='image/jpeg' && $imageinfo['mime']!='image/png' && isset($imageinfo) ){
			$_SESSION[$alert] = $message;
		    header('Location:'.$location);
		    exit(0);
		...
		$uniq_filename = sha1(uniqid(rand()));
		$fileToUpload_name = $uniq_filename.'.'.$file_extension;
	  	return $fileToUpload_name;
	}
